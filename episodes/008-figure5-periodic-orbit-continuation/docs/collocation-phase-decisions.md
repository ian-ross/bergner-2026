...
TASK-075 may proceed under the retained `external-gauss3-hr-adaptive-v1` method, and no method-version revision is required now. This does not authorize interpolation across the remaining 30 unresolved pilot targets; each later full-domain target must still receive one backend-emitted terminal status and preserve Hopf, tripwire, instability, and unresolved-gap boundaries.
