...
- add Figure 3-specific LOCA Hopf continuation orchestration through the pure C++ native Moore--Spence command;
- preserve the same physical equations and normalized output semantics as Python and AUTO;
- document the Trilinos complex-support prerequisite and keep raw C++ stdout/stderr provenance for native LOCA runs.
...
| `loca_native_hopf_stepper` | LOCA-specific boolean; expected `true` only for rows generated by the C++ native Moore--Spence Hopf command |
| `raw_loca_run` | LOCA-specific path to raw C++ stdout/stderr provenance |
...
- `outputs/figure3_loca_hopf_loci/` — NOX/LOCA Figure 3 outputs after TASK-025, generated by the pure C++ native Moore--Spence Hopf command when a complex-enabled Trilinos build is available.
