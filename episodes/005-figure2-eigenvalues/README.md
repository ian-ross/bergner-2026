...
Goal: reproduce Bergner & Spichtinger (2026) Figure 2 eigenvalue curves using three independently generated equilibrium/eigenvalue output paths: Python, AUTO-07p, and a Trilinos-side C++ backend. The C++ backend currently uses Sacado and Teuchos/LAPACK, but it is not yet a full NOX/LOCA continuation implementation.
...
- `outputs/` — curated Python, AUTO, Trilinos-side C++/`loca`-directory, and cross-backend comparison artifacts.
...
- Figure 1 Trilinos-side C++ backend, Sacado residual/Jacobian work, and comparison patterns: `episodes/004-figure1-loca-continuation/`
- Shared AUTO backend/model assets, when reusable: `auto/`
- Shared Trilinos-side C++ backend code, when reusable: `loca/` (directory name retained for now; full NOX/LOCA is deferred)
...
# Generate/normalize Trilinos-side C++ Figure 2 outputs with backend-side physical eigenvalues.
...
- `outputs/figure2_loca_eigenvalues/` — Trilinos-side C++ branch/eigenvalue CSVs, backend-side Sacado physical-Jacobian diagnostics, raw C++ CSV/stderr, summary, metadata, and draft Figure 2-style plot. The directory name is historical; these outputs do not yet exercise NOX/LOCA continuation APIs.
- `outputs/figure2_backend_comparison/` — integrated backend-comparison artifacts:
  ...
  - `figure2_backend_pairwise_differences.csv` — pairwise Python/AUTO/Trilinos-side C++ tracked-eigenvalue differences at each canonical grid point (`loca` remains the historical backend label in CSVs).
  ...
  - `figure2_reproduction_backend_comparison.png` — headline two-panel real/imaginary Figure 2 reproduction with Python, AUTO, and Trilinos-side C++ overlays and Hopf markers.

## Integrated comparison notes

The integrated comparison uses `log_w = ln(w_m_s)` as the canonical coordinate and writes `w_m_s` alongside it. Because the backend runs have different raw grid densities (`python`/Trilinos-side C++: 801 points; `auto`: 411 points in the curated run), `compare_figure2_eigenvalues.py` creates a derived canonical grid over the common backend overlap and linearly interpolates tracked eigenvalue branch real/imaginary parts onto that grid. This alignment is only for comparison products; no raw backend branch/eigenvalue CSV is modified or discarded.
...
Python, AUTO, and the Trilinos-side C++ backend should each generate their own equilibrium/eigenvalue outputs for comparison. Shared package/backend code may be reused, but Episode 5 comparison claims should not come from copying one backend's final eigenvalue table into another backend's output.

The current C++ backend must compute the physical Jacobian and physical eigenvalues backend-side, using Sacado for derivatives and Teuchos/LAPACK where available for dense eigenvalue calculations. AUTO may fall back to Python analytic physical-Jacobian eigenvalues after native AUTO eigenvalue options have been investigated and the fallback is documented in Episode 5 planning notes and run metadata.

A full NOX/LOCA continuation implementation is intentionally deferred. The near-term strategy is to keep using Python, AUTO, and this simple, understandable Trilinos-side C++ backend for additional reproduction tasks, then reimplement the C++ continuation path with NOX/LOCA once the equations, output schemas, and comparison targets are stable.
